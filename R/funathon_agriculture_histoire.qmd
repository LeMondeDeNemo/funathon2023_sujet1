...
total <- mon_canton %>%
  filter(name %in% c("cereales","racines_et_legumes_divers","cultures_diverses","prairies_artificielles","jacheres")) %>% 
...
url_completed <- sprintf(url, "CLERMONT-FERRAND", "63")
...
print(resultat)
print(resultat$centre$coordinates[[1]][1])
```
...
surfaces_ra1852_dep <- surfaces_ra1852 %>% filter(!is.na(departement)) %>% group_by(code, canton) %>% arrange(., desc(code))%>% #filter(canton%in%c("GRASSE","TOULOUSE")) %>%
  mutate(code_dep = ifelse(code %in% c(201, 205), "2A", ifelse(code %in% c(202, 203, 204), "2B",
                    ifelse(code<100, paste0("0",as.character(code %/% 10)), as.character(code %/% 10)) )),
         code_dep = case_when(
           canton == "BELFORT" ~ as.character(90),
           canton == "CHATEAU SALINS" ~ as.character(57),
           canton == "SARREBOURG" ~ as.character(57),
           canton == "BRIEY" ~ as.character(54),
           canton == "SAINT DENIS" ~ as.character(93),
           canton == "SCEAUX" ~ as.character(92),
           canton == "CORBEIL" ~ as.character(91),
           canton == "PONTOISE" ~ as.character(95),
           canton == "ETAMPES" ~ as.character(91),
           canton == "GRASSE" ~ "06",
           TRUE ~ code_dep
         ),
         canton = case_when(
           canton == "CLERMONT" & code == 632 ~ "CLERMONT-FERRAND",
           canton == "LA PALISSE" ~ "LAPALISSE",
           canton == "PONTS L'EVEQUE" ~ "PONT L'EVEQUE",
           canton == "ISSINGEAUX" ~ "YSSINGEAUX",
           canton == "CHALONS SUR MARNE" ~ "Chalons-en-Champagne",
           canton == "CASTEL SARRASIN" ~ "CASTELSARRASIN",
           canton == "AIX" ~ "Aix-en-Provence",
           canton == "NAPOLEONVILLE" ~ "PONTIVY",
           canton == "BRIEY" ~ "VAL DE BRIEY",
           canton == "SCHLESTADT" ~ "SELESTAT",
           canton == "CORBEIL" ~ "Corbeil-Essonnes",
           canton == "NAPOLEON VENDEE" ~ "La Roche-sur-Yon",
           canton == "REMIRMONT" ~ "Remiremont",
           TRUE ~ canton
         ),
         coord_1 = unlist(fromJSON(sprintf(url, gsub(" ","-",canton), code_dep))$centre$coordinates)[1],
         coord_2 = unlist(fromJSON(sprintf(url, gsub(" ","-",canton), code_dep))$centre$coordinates)[2])
...
leaflet(data = surfaces_ra1852_dep) %>% addTiles() %>%
  addMarkers(~coord_1, ~coord_2, popup = ~as.character(canton), label = ~as.character(canton))
...
surfaces_ra1852_dep <-
  st_as_sf(
    surfaces_ra1852_dep %>% drop_na(c(coord_1, coord_2)),
    coords = c("coord_1", "coord_2"),
    crs = 4326
  )

# on charge la projection pour le mettre en Lambert-93
surfaces_ra1852_geo <- st_transform(surfaces_ra1852_dep, crs = 2154)
...
voronoi_surfaces_ra1852_sf <- st_sf(voronoi_surfaces_ra1852)
voronoi_surfaces_ra1852_attributed <- st_join(voronoi_surfaces_ra1852_sf, surfaces_ra1852_geo)

# Visualisation simple
plot(voronoi_surfaces_ra1852_attributed)
```
...
dep_poly <- read_sf(here("data/dep2021_simplify2.json"), crs=2154)

dep_poly <- dep_poly %>% filter(nchar(dep)==2)

voronoi2 <- dep_poly %>% st_union() %>% st_intersection(.,voronoi_surfaces_ra1852_attributed)

voronoi2_sf <- st_sf(voronoi2)
voronoi2_attributed <- st_join(voronoi2_sf, voronoi_surfaces_ra1852_attributed)

# Définir la fonction de palette de couleurs
palette <- colorRampPalette(c("white", "red"))
# Générer la palette de couleurs
colors <- palette(100)  # Ici, 100 est le nombre de couleurs à générer
# Tracer les données

ggplot() +
  geom_sf(data = voronoi2_attributed, aes(fill = voronoi2_attributed$cereales)) +
  scale_fill_gradient(low = "white", high = "red") +
  labs(fill = "Céréales")


#Exportation au format geojson avec le système de coordonnée classique
voronoi2_attributed_classicSystem <- st_transform(voronoi2_attributed, 4326)
st_write(voronoi2_attributed_classicSystem, here("surfaces_ra1852_geo.GeoJSON"))

#Exportation au format csv
coords <- st_coordinates(surfaces_ra1852_geo$geometry)

# Ajouter les coordonnées au dataframe et supprimer l'ancienne
surfaces_ra1852_geo2 <- surfaces_ra1852_geo
surfaces_ra1852_geo2$X <- coords[, 1]
surfaces_ra1852_geo2$Y <- coords[, 2]
surfaces_ra1852_geo2$geometry <- NULL

write.csv(surfaces_ra1852_geo2, here("surfaces_ra1852_geo.csv"), col.names=TRUE)
