...
# Tracer les données
plot(voronoi2_attributed, col = colors)
...
"""#Modification de la valeur d'Albi pour les Céréales pour checker
surfaces_ra1852_dep[37,"canton"][[1]]
valInitiale <- surfaces_ra1852_dep[37,"cereales"]
surfaces_ra1852_dep[37,"cereales"] <- 10000000


#Affichage des Céréales
listing <- surfaces_ra1852_dep$cereales
maxi <- max(listing)
colList <- rgb(listing[1]/maxi,0,0)
for(i in 2:length(listing))
{
  cc <- rgb(listing[i]/maxi,0,0)
  colList <- c(colList,cc)
}
plot(voronoi2_attributed, col=c(rep(colList,361)))


#Affichage des Paturages
listing <- surfaces_ra1852_dep$paturages
maxi <- max(listing)
colList <- rgb(listing[1]/maxi,0,0)
for(i in 2:length(listing))
{
  cc <- rgb(0,listing[i]/maxi,0)
  colList <- c(colList,cc)
}
plot(voronoi2_attributed, col=c(rep(colList,361)))"""
